...
const R_CODES = L_CODES.map((l) => l.split("").map((b) => (b === "0" ? "1" : "0")).join(""));
 ...
 */
export async function generateEan13(exists) {
...
function barsToSvg(bars, height, color, scale) {
    return bars
        .map((b) => `<rect x="${b.x * scale}" y="0" width="${b.width * scale}" height="${height}" fill="${color}"/>`)
        .join("");
}
/**
 ...
 */
export function ean13Svg(value, opts = {}) {
    const digits = value.replace(/\D/g, "");
...
export function code39Svg(value, opts = {}) {
    ...
    for (const bit of pattern)
        modules += bit === "1" ? "11" : "1"; // wide = 2× narrow
    const bars = [];
    let i = 0;
    while (i < modules.length) {
        if (modules[i] === "1") {
