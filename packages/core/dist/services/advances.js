...
export async function createAdvance(db, input) {
    if (input.amount <= 0)
        throw new AdvanceError("Amount must be positive", "INVALID_AMOUNT");
    const party = await db.query.parties.findFirst({ where: eq(schema.parties.id, input.partyId) });
    if (!party)
        throw new AdvanceError("Party not found", "NOT_FOUND", 404);
    const [row] = await db
        ...
        .returning();
    await logActivity(db, input.direction === "GIVEN" ? "ADVANCE_GIVEN" : "ADVANCE_TAKEN", `${input.direction === "GIVEN" ? "Gave" : "Received"} advance of ${input.amount} to/from "${party.name}"${input.note ? ` — ${input.note}` : ""}`);
    return row;
}
export async function listAdvances(db, partyId) {
    return db
        .select()
...
export async function settleAdvance(db, id) {
    const [row] = await db
        ...
        .returning();
    if (!row)
        throw new AdvanceError("Advance not found", "NOT_FOUND", 404);
...
export async function recordPayment(db, input) {
    ...
    const [row] = await db
        ...
        .returning();
    await logActivity(db, input.direction === "IN" ? "PAYMENT_IN" : "PAYMENT_OUT", `${input.direction === "IN" ? "Received" : "Paid out"} ${input.amount}${input.note ? ` — ${input.note}` : ""}`);
    return row;
}
export async function listPayments(db, partyId) {
    return db
        .select()
