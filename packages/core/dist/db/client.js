...
export function createDb(opts) {
    ...
    const host = opts?.host ?? (typeof process !== "undefined" ? process.env?.NEON_HOST : undefined);
    const user = opts?.user ?? (typeof process !== "undefined" ? process.env?.NEON_USER : undefined);
    const password = opts?.password ?? (typeof process !== "undefined" ? process.env?.NEON_PASSWORD : undefined);
    ...
    const resolvedHost = host ?? (databaseUrl ? parseConnectionString(databaseUrl).host : undefined);
    const resolvedUser = user ?? (databaseUrl ? parseConnectionString(databaseUrl).user : undefined);
    const resolvedPassword = password ?? (databaseUrl ? parseConnectionString(databaseUrl).password : undefined);
    if (!resolvedHost) {
        throw new Error("Munim core: no database configured. Set DATABASE_URL (or NEON_HOST/USER/PASSWORD).");
    }
    ...
    if (databaseUrl) {
        ...
        throw new Error("Munim core: no database credentials. Set DATABASE_URL (or NEON_HOST/USER/PASSWORD, or DATABASE_AUTH_TOKEN).");
    }
    const endpoint = `https://${resolvedHost}/sql`;
    const callback = async (sql, params) => {
...
export function getDb(opts) {
    if (!globalForDb.munimDb)
        globalForDb.munimDb = createDb(opts);
