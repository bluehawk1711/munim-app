...
export const defaultJobLetterData: JobLetterData = {
 ...
 */
export function jobLetterFromStored(
  stored: Record<string, unknown> | null,
  row: JobLetterRowLike,
  company?: JobLetterCompanyFallback,
): JobLetterData {
  const s = stored ?? {};
...
export function formatJoiningDate(iso: string): string {
  if (!iso) return "________ (Joining Date)";
  const date = new Date(`${iso}T00:00:00`);
  if (Number.isNaN(date.getTime())) return "________ (Joining Date)";
 ...
 */
export function renderJobLetterHtml(data: JobLetterData): string {
