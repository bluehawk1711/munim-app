...
export function PinSettingsCard() {
    ...
    return (_jsxs(Card, { children: [_jsxs(CardHeader, { children: [_jsxs(CardTitle, { className: "flex items-center gap-2 text-sm", children: [_jsx(KeyRound, { className: "h-4 w-4" }), " App lock"] }), _jsx(CardDescription, { className: "text-xs", children: "A 4-digit PIN unlocks this device \u2014 stored locally (hashed), never sent to the database." })] }), _jsxs(CardContent, { className: "space-y-4", children: [_jsxs("div", { className: "flex flex-wrap items-center gap-2", children: [enabled ? (_jsxs(Badge, { className: "gap-1", children: [_jsx(ShieldCheck, { className: "h-3 w-3" }), " PIN lock enabled"] })) : (_jsxs(Badge, { variant: "secondary", className: "gap-1", children: [_jsx(ShieldOff, { className: "h-3 w-3" }), " Lock disabled"] })), lock.isTestAccount && (_jsx(Badge, { variant: "secondary", children: "Test account \u2014 PIN 1234" }))] }), enabled ? (_jsxs(_Fragment, { children: [_jsxs("div", { className: "space-y-2", children: [_jsx(PinFields, { current: current, newPin: newPin, confirm: confirm, onChange: (f, v) => {
                                            if (f === "current")
                                                setCurrent(v);
                                            if (f === "newPin")
                                                setNewPin(v);
                                            if (f === "confirm")
                                                setConfirm(v);
                                        } }), _jsxs(Button, { onClick: handleChange, disabled: busy, size: "sm", className: "gap-1.5", children: [_jsx(KeyRound, { className: "h-3.5 w-3.5" }), " Change PIN"] })] }), _jsxs("div", { className: "flex flex-wrap gap-2 border-t pt-4", children: [_jsxs(Button, { variant: "outline", size: "sm", onClick: handleDisable, disabled: busy, className: "gap-1.5", children: [_jsx(ShieldOff, { className: "h-3.5 w-3.5" }), " Disable lock"] }), _jsxs(Button, { variant: "ghost", size: "sm", onClick: handleResetToTest, className: "gap-1.5 text-muted-foreground", children: [_jsx(RotateCcw, { className: "h-3.5 w-3.5" }), " Reset to test account"] })] })] })) : (_jsxs("div", { className: "space-y-2 border-t pt-4", children: [_jsx("p", { className: "text-xs text-muted-foreground", children: "Enable the lock with a new 4-digit PIN. You'll be asked for it next launch." }), _jsx(PinFields, { current: "", newPin: newPin, confirm: confirm, showCurrent: false, onChange: (f, v) => {
                                    if (f === "newPin")
                                        setNewPin(v);
                                    if (f === "confirm")
                                        setConfirm(v);
