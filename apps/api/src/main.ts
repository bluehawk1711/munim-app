...
async function bootstrap(): Promise<void> {
  ...
  if (origins.length > 0) {
    app.enableCors({ origin: origins, methods: ["GET", "POST", "PUT", "PATCH", "DELETE"] });
  }
  ...
  Logger.log(
    `Munim API listening on http://0.0.0.0:${env.PORT} (${env.NODE_ENV}, CORS: ${origins.length > 0 ? origins.join(", ") : "none"})`,
    "Bootstrap",
  );
}
