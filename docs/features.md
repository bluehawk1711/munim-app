...
- **One settings layout.** Web + desktop share `SettingsShell` in `@munim/ui` (sectioned sidebar
  layout — Shop / Appearance / Security / Server). Any settings page change is made once and
...
|---|---------|:---:|:-------:|:------:|-------|
| 1 | Dashboard (revenue, receivables, payables, low stock, recent invoices/advances) | ✅ | ✅ | ✅ | Same `getDashboard` in core. Charts: **Monthly Sales** (area), **Stock Distribution** (donut), **Top Selling Products** (bar), **Invoice Status** (donut), **Sales by Category** (donut), **Advances Given vs Taken** (donut), **Units Sold per Month** (bar) — web renders recharts; desktop renders **bklit charts** (shadcn registry: Area/Bar/Pie/Line/Radar + Legend in `apps/desktop/src/components/charts/` — dashboard has the revenue **AreaChart** / orders **LineChart** toggle + category **RadarChart** with interactive Legend; reports has trajectory AreaChart + Category **PieChart** donut + Units-sold BarChart; inventory has Stock-Allocation BarChart + Category-Value donut; catalog has a Linked-Products donut); mobile renders **`react-native-gifted-charts`** (`apps/mobile/src/components/charts.tsx`: `LineChart` curved-area + crosshair pointer, `BarChart`, `DonutChart` — same stable exports, colored from the active theme's `chart1..chart5` + status tokens, mode-aware; text-only cards on Home/Sales/Advances were upgraded to live donut/bar charts). **Mobile home is a counter-style dashboard**: custom `HomeHeader` (shop-name eyebrow, big title, search/scan/profile actions — **scan is Home-only** and opens the shared `BarcodeScannerModal`; a found barcode opens the `QuickSaleSheet` instant-sale dialog), status chips, **TOTAL NET REVENUE hero + 6-month performance line chart with MoM delta**, quick actions (Record Sale / Add Stock / New Khata / Reports), **Khata Net Position** card (You Pay vs You Receive split bar from the real `payables`/`receivables` ledger split), Recent Counter Activity (deep-links to filtered invoices) and a **Create New Bill / Order** CTA. KPI chips deep-link: chips → Stock/Sales tabs or More → Invoices (with the matching status filter). Data added to `DashboardStats` in core (`topProducts`, `salesByCategory`, `invoiceStatus`, `advanceSplit`, `soldPerMonth`). **Desktop reaches the API through `@tauri-apps/plugin-http`** (Rust fetch, no CORS) via `lib/api.ts`; mobile uses global fetch (native, no CORS). **Desktop dashboard is a counter-style suite**: quick-action bar (New Bill / Quick Sale / Stock & Barcode / Khata Entry), 6 hero stat tiles with a Stock Alert meter, **Turnout & Category Trajectory** card (Gifted-area revenue chart / orders line chart with Monthly/Quantity toggle + category radar + interactive legend), **Khata & Karigar Position** card (net outstanding, Given/Took split bar, active advances), **Top Moving Products** with product-image thumbnails, and a **Recent Settlement Invoices** table (paged from the shared list API, per-row PDF download via the shared `generateBillPDF`). **All desktop pages share the analytics-suite shell**: a `PageHeader` band (big title + uppercase pill badge + subtitle + page actions) — Products (Inventory, low/out chips), Catalog (Variants, total count), Sales (Counter), New Bill (Billing), Invoices (Ledger), Advances (Cash Flow), Job Letters (HR Suite), Reports, Parties (Active Ledger) |
| 2 | Products — list, search, create, edit, delete | ✅ | ✅ | ✅ | SKU auto-generated in core. Mobile has search (name/SKU/color/size, client-side over the full list with a clear button) and a shared add/edit form (`updateProduct` on save) alongside create, delete, adjust stock + image upload. **Mobile inventory is a swatch-row list**: custom header, search + scan row, stock filter chips with live counts (All / In / Low / Out), image-forward rows (thumbnail, name, variant chip with weight, price + cost line, stock pill), and tapping a row opens a **product detail bottom sheet** (`@gorhom/bottom-sheet`: SKU, barcode Copy card, Buy/Sell/Margin tiles, detail rows, Edit Product / Adjust Stock actions). **Barcode scan → instant sale**: the scan row opens the shared `BarcodeScannerModal`, and a found product opens the `QuickSaleSheet` (quantity, price override, optional walk-in customer, payment-received switch) wired to the shared `useCreateSale` quick-sale mutation. **Purity field end-to-end** (migration 0004): products carry a metal-purity string (e.g. "22K" / "916") — editable in the add/edit form on all 3 platforms (web form dialog, desktop form + table column, mobile form), and shown in web products table, desktop meta line, and the shared details dialog / mobile detail sheet |
...
| 4 | Catalog — colors & sizes management (add/rename/delete) | ✅ | ✅ | ✅ | Shared `catalog.ts` service in core (`listCatalogItems`/`createCatalogItem`/`renameCatalogItem`/`deleteCatalogItem` with product-count guards); all three apps manage the same colors/sizes. Mobile renders the swatch-card layout: search, segmented pill tabs (Colors/Sizes/Categories), per-item card with `swatchColor` tile + hex chip, "N products linked" subtitle, In-Use/Unused badge, and 3-dot rename/delete (delete blocked while in use) |
| 5 | Sales — quick sale (product, qty, price, customer, paid/unpaid) | ✅ | ✅ | ✅ | `createSale` in core; web + desktop also share search, date-range filter, summary tiles and **undo sale** (stock restore via `deleteInvoice`). **Mobile Sales is a khata net-status view**: custom header, **You'll Get / You'll Give** tiles + net-owed strip (from the shared `usePartyBalances` ledger split — balance > 0 = they owe us), filter chips (All / You'll Get / You'll Give / Settled), expandable party cards with per-party advance totals (given/taken open advances), shareable khata statement (real `Share` of all balances), and an **Add New Party** CTA (`useCreateParty`) — payment recording and advances stay in Khata/Advances where those flows live |
| 6 | Billing / Invoice creation (line items, discount, delivery, paid-now, date, notes, party link) | ✅ | ✅ | ✅ | Shared `buildBillDocument`; the **template options (template / classic color / 2-in-1 duplicate-separate) are driven by the same model on all 3 platforms**: web + desktop render the shared `BillTemplateOptions` component in `@munim/ui`; mobile renders native segmented controls (Classic Jewellery / Modern E-commerce, red-yellow swatches, 2-in-1 switch, Duplicate / Separate) with a full second-bill editor in Separate mode. The template types (`BillTemplate`/`BillClassicColor`/`BillMode`) live in `@munim/core` (re-exported by `@munim/ui`) and each invoice snapshots its settings in `template_settings`. **Desktop "On save" options:** checkboxes for **Auto-save the PDF after creating** (runs the shared `generateBillPDF` right after the invoice lands) and **Mark as paid** (saves the invoice with its full total as `amountPaid` → status PAID instead of UNPAID; wins over the Paid-now field, applies to both bills in Separate mode). **Bill-to/details parity:** all three forms collect customer name + phone + address, **link-to-khata-party** (mobile: bottom-sheet picker that pre-fills name/phone/address, mirroring web's select), **date** (mobile: native date picker — Android dialog / iOS inline sheet, no manual typing), and **notes/terms** (mobile: multi-line field); date + notes apply to both bills in 2-in-1 Separate mode, exactly like web's shared `basePayload`. **Field order & live totals (all 3 platforms):** Customer → Items → Discount / Delivery / Paid-now (below the items they adjust) → Notes — with a **live totals strip** (Subtotal / − Discount / + Delivery / = Total / Paid / Balance due) on desktop (`TotalsSummary`), web (inside `BillItemsCard`'s charges row) and mobile (`TotalsBlock`). **Picking a product auto-fills its selling price** on all platforms (mobile also carries SKU/color/size onto the line). **0-rupee bills are blocked end-to-end**: `createInvoice` in core rejects empty items, qty < 1 (`BAD_QUANTITY`), price ≤ 0 (`ZERO_PRICE_ITEM`) and total ≤ 0 after discount (`ZERO_TOTAL`); every client pre-validates with clear toasts/alerts (mobile's old silent `return` on empty items now alerts). Verified against the live DB via `packages/core/scripts/verify-invoice-guards.ts` |
...
| 11 | Parties & Khata — balances (due / owed), ledger, advances given/taken | ✅ | ✅ | ✅ | Web + desktop share search + type filter (customer/supplier/worker/other), party type on create, delete party, and the shared `LedgerKindBadge` in ledger rows; mobile shows balances + compact ledger. **Desktop khata is a ledger suite**: summary tiles (Net Receivables / Net Payables / Active Credit Accounts / Settled), filter chips with counts (All / You Will Get / You Will Give / Karigars / Settled), a paginated **Khata Ledger Directory** table (avatar initials, contact, category chip, on-ledger-since, net balance direction), and a right **party panel** (net position, advances given/taken chips, Give Advance / Money in / Money out actions via the shared `KhataActionDialog`, recent ledger entries with kind badges, per-advance Settle) |
| 12 | Advances summary — "whom I gave money / whom I owe" dashboard | ✅ | ✅ | ✅ | Web + desktop share the same **Advances page** (`advances-view.tsx` / `pages/advances.tsx`) built on the shared `SummaryTile` + `KhataCard` from `@munim/ui`. **Desktop is the Advances & Cash Movement suite**: header band with Export Day Book (CSV), 4 tiles (We will receive / We owe / Net Position / Open Advances Outstanding) with a live ledger stats strip, Quick Advance entry (shared `QuickAdvanceRecord`), the two KhataCard columns (Whom I Gave Advance / Whom I Still Have To Give), and a **Recent Cash Movement & Daybook** table — every advance with party name + type, direction badge, signed amount, Open/Settled status, per-row **Settle** (`useSettleAdvance`) and Delete (`useDeleteAdvance` with confirm), All/Given/Taken/Open filter chips and pagination (8/page) with given/taken/open totals in the footer. Mobile has a dedicated **`AdvancesScreen`** (More → Advances) — summary tiles + a receive-vs-pay **donut** (Gifted Charts) with net in the center, quick record, and receivables/payables lists |
| 13 | Settle advance | ✅ | ✅ | ✅ | Shared `settleAdvance` in core; mobile has a Settle button per open advance in Parties |
| 14 | Reports — daily/weekly/monthly/yearly/stock/low-stock/sold (+ custom dates) | ✅ | ✅ | ✅ | Shared `getReport` in core; all three apps generate + export the same report. Custom date range applies to **any** report type on all three apps — dates are committed on "Generate" and an empty range falls back to the type's default period. Mobile uses a **native date picker** (`@react-native-community/datetimepicker`: Android dialog / iOS inline sheet) in Reports and Billing instead of typed YYYY-MM-DD text. **Desktop Reports is an analytics suite**: header band with Export CSV + refresh, report-window bar (optional date range + Generate), report-type pill tabs, four hero tiles (Total Turnout, Gross Profit with margin, Metal Weight Dispatched, Items Needing Action), a 6-month **Revenue Trajectory** bar chart from the shared dashboard `monthlySales` (peak badge + monthly average), a **Category Split** card, and an **Itemized Performance & Margin Audit** table (SKU filter, cost basis, realized profit + %) with the totals strip. **Mobile Reports is an Analytics Suite layout**: custom header, report-type pills, REPORT WINDOW card (period label + Edit range + Run Report / Share CSV), GROSS SALES + EST. PROFIT hero tiles (with live margin %), a TURNOVER TRAJECTORY line chart from the shared dashboard `monthlySales` (sales-type reports), and sorted Itemized Records (Revenue / Profit / Qty pills; per-product revenue, sold/stock, cost basis, profit + %) |
...
| 22 | Product label printing (preview, copies, print / PDF, 24-up A4 sheet; **direct thermal on desktop**) | ✅ | ✅ | ✅ | One model in core: `buildProductLabel` + `renderLabelSheetHtml` (63.5 × 33.9 mm labels, 3×8 grid, inline SVG barcode — the SAME markup on every platform). Web + desktop share the `LabelPrintDialog` from `@munim/ui` (label preview, copies stepper, Print via browser dialog + Download PDF via jsPDF `html()` at true physical size); mobile prints via `expo-print` + share. **Desktop extra:** direct printing to a thermal barcode printer (TSC TE244 etc.) — core `buildLabelTspl2` emits native TSPL2 commands (EAN-13/Code 128 drawn by the printer, 203 dpi), Rust `printer.rs` spools them raw to the Windows print queue (no dialog); printer chosen in Settings → Printing (`munim.labelPrinter`, device-local) or in the dialog. Web omits the `directPrint` prop (browser sandbox can't raw-print) |
...
- Settings: `SettingsShell` sectioned layout (Shop profile / Appearance / Security / Server) via `GET/PUT /api/settings` + connection check (API URL/key come from build-time env on web, saved per-device on desktop); header shows only the light/dark toggle (color theme lives in Settings)
...
- Settings: same shared `SettingsShell` sectioned layout as web (Shop profile / Appearance / Security / **Printing** / Server) — URL + key fields are masked (password-style with show/hide eye); test pings `GET /readyz`; only the saved host is shown. **Printing section** lists installed printers (Rust `list_printers`), saves the label printer (`munim.labelPrinter`, device-local) and prints a test label
...
- Products: image upload via the shared API (`api.upload.image` — server-side Cloudinary signing, no client secret) + thumbnail column; weight (mg) field; inline barcode display; **barcode lookup input** (USB scanners) + **Generate missing barcodes**; **Print label** per product (shared `LabelPrintDialog` → direct thermal TSPL2 print / A4 Print / jsPDF download via `lib/labelPdf.ts`)
- **Label printer (thermal)**: Rust commands `list_printers` + `print_raw` (`src-tauri/src/printer.rs`, winspool RAW datatype) — TSC TE244 and other TSPL2 printers print native barcodes with no print dialog; TSPL2 stream built by core `buildLabelTspl2`
