...
| 17 | Connection (web: `NEXT_PUBLIC_API_URL`/`NEXT_PUBLIC_API_KEY` env; desktop + mobile: API URL + key) | ✅ | ✅ | ✅ | **All three apps** connect to the shared **NestJS API** through `@munim/api-client`. Desktop + mobile store the base URL (masked) + optional key override (build-time `VITE_API_KEY` / `EXPO_PUBLIC_API_KEY` fallback), test via `GET /readyz` (non-dismissible modal), and show an "Open Settings" CTA when unconfigured. Onboarding is a single API-URL (+ optional key) step — the API proxies database AND Cloudinary, so no Neon URL or Cloudinary secrets live on the device. Web reads `NEXT_PUBLIC_API_URL` + `NEXT_PUBLIC_API_KEY` at build time; its Settings connection check pings the API through the shared client |
