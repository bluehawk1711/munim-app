...
| 14 | Reports — daily/weekly/monthly/yearly/stock/low-stock/sold (+ custom dates) | ✅ | ✅ | ✅ | Shared `getReport` in core; all three apps generate + export the same report. Custom date range applies to **any** report type on all three apps — dates are committed on "Generate" and an empty range falls back to the type's default period |
