...
- **One settings layout.** Web + desktop share `SettingsShell` in `@munim/ui` (sectioned sidebar
  ...
  (dashboard → settings) exist on all three platforms (no ❌ rows). Mobile Products is 🟡 —
  create/delete/stock-adjust/image-upload are there, but **search + edit aren't on mobile yet**
  (web + desktop have them).
...
| 3 | Stock — adjust (+/− with reason), low-stock/out-of-stock badges | ✅ | ✅ | ✅ | `adjustStock` + movements in core (input supports `reason`). Web + desktop show a Reason field on the adjust dialog; **mobile adjusts +/− without a reason note yet** |
...
| 14 | Reports — daily/weekly/monthly/yearly/stock/low-stock/sold (+ custom dates) | ✅ | ✅ | ✅ | Shared `getReport` in core; all three apps generate + export the same report. Custom date range: web + desktop apply it to any report type; **mobile currently applies it only to the Sold report** |
